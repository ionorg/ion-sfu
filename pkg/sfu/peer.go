...
func (p *Peer) Join(sid, uid string) error {
	...
	p.subscriber.OnNegotiationNeeded(func() {
		...
		debugLogger.Info("Negotiation needed", "peer_id", p.id)
		offer, err := p.subscriber.CreateOffer()
		if err != nil {
			defaultLogger.Error(err, "CreateOffer error")
			return
		}

		p.remoteAnswerPending = true
		if p.OnOffer != nil && !p.closed.get() {
			defaultLogger.Info("Send offer", "peer_id", p.id)
			p.OnOffer(&offer)
		}
	})

	p.subscriber.OnICECandidate(func(c *webrtc.ICECandidate) {
		debugLogger.Info("On subscriber ice candidate called for peer", "peer_id", p.id)
		if c == nil {
			return
		}
	...
	p.publisher.OnICECandidate(func(c *webrtc.ICECandidate) {
		debugLogger.Info("on publisher ice candidate called for peer", "peer_id", p.id)
		if c == nil {
			return
		}
	...
	defaultLogger.Info("Peer join session", "peer_id", p.id, "session_id", sid)
...
func (p *Peer) Answer(sdp webrtc.SessionDescription) (*webrtc.SessionDescription, error) {
	...
	defaultLogger.Info("Peer got offer", "peer_id", p.id)
	...
	defaultLogger.Info("Peer send answer", "peer_id", p.id)
...
func (p *Peer) SetRemoteDescription(sdp webrtc.SessionDescription) error {
	...
	defaultLogger.Info("Peer got answer", "peer_id", p.id)
...
func (p *Peer) Trickle(candidate webrtc.ICECandidateInit, target int) error {
	...
	defaultLogger.Info("Peer trickle", "peer_od", p.id)
	switch target {
	case publisher:
